...
_punctuation = " "
_special = ""
_letters = 'ABDEFGHIJKLMNOPQRSTUVXYZabdefghijklmnopqrstuvxyz'
...
# Export all symbols:
AGHA_SYMBOLS = (
        [_pad] + list(_special) + list(_punctuation) + list(
    _letters) + _arpabet + [_eos]
)
...
@dataclass
class AghaProcessor(BaseProcessor):
    """Agha processor."""
    ...
    def create_items(self):
        if self.data_dir:
            with open(
                    os.path.join(self.data_dir, self.train_f_name),
                    encoding="utf-8"
            ) as f:
                self.items = [self.split_line(self.data_dir, line, "|")
                              for line in f]
    ...
    def _symbols_to_sequence(self, symbols):
        return [self.symbol_to_id[s] for s in symbols if
                self._should_keep_symbol(s)]
